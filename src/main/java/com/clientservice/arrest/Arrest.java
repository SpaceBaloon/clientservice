...
@Table( name = "AREST", 
        uniqueConstraints = @UniqueConstraint( columnNames = { "AGENCY_ID", "DATE", "NUMBER", "CLIENT_ID" } )
)
public class Arrest implements Serializable {
    ...
    @NotNull( message = "Arrest.agency {validation.notNull}" )
    @OneToOne
    @JoinColumn( name = "AGENCY_ID" )
    private Agency agency;
    
    @NotNull( message = "Arrest.date {validation.notNull}" )
    @Column( name = "DATE" )
    ...
    @NotBlank( message = "Arrest.number {validation.notEmpty}" )
    ...
    @NotNull( message = "Arrest.amount {validation.notNull}" )
    @Min( value = 0, message = "Arrest.amount {validation.min}" )
    @Column( name = "AMOUNT", precision = 18, scale = 0 )
    private BigDecimal amount;
    
    @NotNull( message = "Arrest.status {validation.notNull}" )
    @Enumerated( EnumType.STRING )
    @Column( name = "STATUS" )
    private ArestStatus status;
    
    @NotNull( message = "Arrest.client {validation.notNull}" )
    @ManyToOne
